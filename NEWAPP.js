...
var guilddata = JSON.parse(fs.readFileSync('storage/guilddata.json', 'utf8'));
var birthdaysData = JSON.parse(fs.readFileSync('storage/birthdays.json', 'utf8'));
const isValidCommand = (message, cmdName, guildID) => message.content.toLowerCase().startsWith("+" + cmdName);
...
await console.log("Promise was rejected. ", error)
})

//Shard disconnection handler
client.on('sharddisconnect', async (shardEvent, number) =>{
await console.log("Websocket Shard Disconnecting... ", shardEvent, number);
});

//Shard error handler
client.on('shardError', async (error, number) =>{
await console.log("Websocket Shard error ", error, number);
});

//Ensure the client is ready before starting to react to commands.
...
//     //Member role update logging
//     client.on('guildMemberUpdate', async (OldMem, NewMem) =>{
//       await wait (1000).then(async () =>{
//         const auditLogs = await OldMem.guild.fetchAuditLogs({limit:1, type:'MEMBER_ROLE_UPDATE'});
//         if(!auditLogs) return;
//         const roleLog = await auditLogs.entries.first();
//         if (roleLog.target.id === NewMem.id){
//           var embed = new MessageEmbed()
//           .setTitle("Role Updated")
//           .setDescription(`Roles for ${OldMem} have been updated`)
//           .addField("Old Roles", OldMem.roles.cache.map(r => `${r}`).join(' | '))
//           .addField("New Roles", NewMem.roles.cache.map(r => `${r}`).join(' | '));
//           await OldMem.guild.channels.cache.get(guilddata[OldMem.guild.id].memberlogChannel).send(embed);
//         }
//       });
//     });
...
//         const inviteUsed = await guildInvites.find(i => ei.get(i.code).uses < i.uses);
//         const inviter = await client.users.cache.get(inviteUsed.inviter.id);
//         var embed = new MessageEmbed()
//         .setTitle("Member Joined")
//         .setColor('#00ff00')
//         .setDescription(`${member} Joined the server using invite code ${inviteUsed.code} which now has ${inviteUsed.uses} uses.`)
//         await inviteUsed.guild.channels.cache.get(guilddata[inviteUsed.guild.id].joinlogChannel).send(embed);
//       })
//     });
  
//     //Invite Creation Logging
//     client.on(`inviteCreate`, async (invite) =>{
//       if(invite.guild.id === "787119026996248586"|| invite.guild.id === "789227902839422986" ) return;
//       var embed = new MessageEmbed()
//       .setTitle("Invite Created")
//       .addField("Invite Code", invite.code, true)
//       .addField("Inviter", invite.inviter, true);
//       if((invite.maxAge/60/60) === 0){
//         await embed.addField("Expiry", `Never`, true)
//       .addField("Max Uses", invite.maxUses, true)
//       .addField("Temporary?", `No`, true);
//       }
//       else{
//         await embed.addField("Expiry", `${invite.maxAge/60/60} hours`, true)
//       .addField("Max Uses", invite.maxUses, true)
//       .addField("Temporary?", `Yes`, true);
//     }
//       await embed.addField("Invite Channel", invite.channel, true)
//       .setThumbnail(invite.inviter.avatarURL({size: 128, dynamic: true}))
//       .setFooter(`Member ID: ${invite.inviter.id} | Time: ${invite.createdAt.toTimeString().substring(0, 17)}`)
//       .setColor(3066993);
//       await invite.guild.fetchInvites().then(guildInvites =>{
//         invites[invite.guild.id] = guildInvites;
//       });
//       await invite.guild.channels.cache.get(guilddata[invite.guild.id].invitelogChannel).send(embed);
//     });
  
//     //Invite Deletion Logging
//     client.on('inviteDelete', async (invite) =>{
//       if(invite.guild.id === "787119026996248586"|| invite.guild.id === "789227902839422986" ) return;
//       const auditEntry = await invite.guild.fetchAuditLogs({type:'INVITE_DELETE'}).then(audit => audit.entries.first())
//       var embed = new MessageEmbed()
//       .setTitle("Invite Deleted")
//       .addField("Invite Code", invite.code, true)
//       .addField("Invite Channel", invite.channel, true)
//       .addField("Deleted by", auditEntry.executor)
//       .setFooter(`Deleter ID: ${auditEntry.executor.id} | Time: ${auditEntry.createdAt.toTimeString().substring(0, 17)}`)
//       .setColor(3066993);
//       await invite.guild.fetchInvites().then(guildInvites =>{
//         invites[invite.guild.id] = guildInvites;
//       });
...
//       if(message.guild.id === "787119026996248586"|| message.guild.id === "789227902839422986" || message.author.bot) return;
//       const logchannel = message.guild.channels.cache.get(guilddata[message.guild.id].msglogChannel);
//         var embed = new MessageEmbed()
//         .setTitle(`Message Deleted | ${message.author.tag}`)
//         .addField("Channel", message.channel, true)
//         .addField("Message", message.content)
//         .setColor("#ff0000");
//         await message.guild.channels.cache.get(guilddata[message.guild.id].msglogChannel).send(embed);
//     });
  
//     //Message Edit Logging
//     client.on('messageUpdate', async (oldMessage, newMessage) => {
//       if(newMessage.guild.id === "787119026996248586"|| newMessage.guild.id === "789227902839422986" || newMessage.author.bot) return;
//       if(newMessage.author.bot) return;
//       if (oldMessage.content === newMessage.content) return;
//       const logchannel = newMessage.guild.channels.cache.get(guilddata[newMessage.guild.id].msglogChannel);
//         var embed = new MessageEmbed()
...
//     //Ban Logging
//     client.on('guildBanAdd', async (guild, user) =>{
//       if (guild.id === "787119026996248586"|| guild.id === "789227902839422986" ) return;
//       await wait(10000).then(async () =>{
//         const AuditEntries = await guild.fetchAuditLogs({limit:1, type:'MEMBER_BAN_ADD'});
//         const BanLog = await AuditEntries.entries.first();
//         if (!BanLog) return await guild.channels.cache.get(guilddata[guild.id].memberlogChannel).send(`${user} was banned but no relevant audit logs could be found`);
//         const {executor, target, reason} = BanLog;
//         var embed = new MessageEmbed()
//         .setTitle("Member Banned")
//         .addField('Member', user)
...
//         .addField('Reason', reason);
//         if(target.id === user.id){
//           await guild.channels.cache.get(guilddata[guild.id].memberlogChannel).send(embed);
//           await client.channels.cache.get("787269455368683540").send(embed);
//         }
//         else{
//           await client.channels.cache.get("787269455368683540").send(embed);
//           await guild.channels.cache.get(guilddata[guild.id].memberlogChannel).send(`${user} was banned but audit logs are inconclusive`);
//         }
//       });
//     });
...
//       if (guild.id === "787119026996248586"|| guild.id === "789227902839422986" ) return;
//       await wait(10000).then(async () =>{
//         const AuditEntries = await guild.fetchAuditLogs({limit:1, type:'MEMBER_BAN_REMOVE'});
//         const BanLog = await AuditEntries.entries.first();
//         if (!BanLog) return await guild.channels.cache.get(guilddata[guild.id].memberlogChannel).send(`${user} was banned but no relevant audit logs could be found`);
//         const {executor, target, reason} = BanLog;
//         var embed = new MessageEmbed()
...
//       if (guildMember.guild.id === "787119026996248586"|| guildMember.guild.id === "789227902839422986" ) return;
//       await wait(10000).then(async () =>{
//         const AuditEntries = await guild.fetchAuditLogs({limit:1, type:'MEMBER_KICK'});
//         const BanLog = await AuditEntries.entries.first();
//         if (!BanLog){
//           var embed = new MessageEmbed()
//           .setTitle("Member Left")
//           .setDescription(`${guildMember} has left the server`)
//           return await guild.channels.cache.get(guilddata[guild.id].memberlogChannel).send(`${user} was banned but no relevant audit logs could be found`);
//         }
//         const {executor, target, reason} = BanLog;
//         if(target.id === user.id){
//           var embed = new MessageEmbed()
//           .setTitle("Member Kicked")
//           .addField('Member', guildMember)
